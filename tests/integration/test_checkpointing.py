...
def dependencies() -> tuple[DeterministicIntentClassifier, OrderLookupTool]:
    repository = JsonOrderRepository(
        Path(__file__).parents[2] / "app" / "data" / "orders.json"
    )
    return DeterministicIntentClassifier(), OrderLookupTool(repository)
...
async def test_workflow_state_survives_sqlite_saver_reopen(tmp_path: Path) -> None:
    ...
    classifier, order_lookup = dependencies()

    async with sqlite_checkpointer(database_path) as saver:
        graph = build_support_graph(classifier, order_lookup, saver)
    ...
    async with sqlite_checkpointer(database_path) as saver:
        ...
        assert len(persisted.values["steps"]) == 4
